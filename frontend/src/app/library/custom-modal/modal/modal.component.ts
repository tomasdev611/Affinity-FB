...
export class ModalComponent implements OnInit, AfterViewChecked {
  ...
  @Output() modalResized: EventEmitter<boolean> = new EventEmitter();

  @ViewChild('modalRoot', {static: false} as any) modalRoot: ElementRef;
  @ViewChild('modalBody', {static: false} as any) modalBody: ElementRef;
  @ViewChild('modalHeader', {static: false} as any) modalHeader: ElementRef;
  @ViewChild('modalFooter', {static: false} as any) modalFooter: ElementRef;
  ...
  rightBottom() {
    ...
    const x = Math.max(window.innerWidth - elementWidth - 10, 0);
    const y = Math.max(window.innerHeight - elementHeight - 10, 0);
  ...
  onResizeEnd(event: ResizableEvent) {
    if (event.direction === 'vertical') {
      this.calcBodyHeight();
    } else {
      window.dispatchEvent(new Event('resize'));
