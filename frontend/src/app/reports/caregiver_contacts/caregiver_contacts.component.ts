...
import {DEFAULT_CLIENT_COLUMNS, CLIENT_COLUMNS} from '../../common/constants';
...
@Component({
  selector: 'app-reports-caregiver-contacts',
  templateUrl: './caregiver_contacts.component.html',
  styleUrls: ['./caregiver_contacts.component.scss']
})
export class CaregiverContactsComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    if (!localStorage.getItem('caregiver_contacts_columns')) {
      localStorage.setItem(
        'caregiver_contacts_columns',
        JSON.stringify([
    ...
    this.pageInfo$ = this.store.pipe(
      ...
      select(fromRoot.getComplianceCaregiverContacts)
    );
    ...
    this.$commonInfo.subscribe(data => {
      if (data.loaded) {
        ...
        if (this.commonCaregiveres !== data.caregivers) {
          this.commonCaregiveres = data.caregivers;
    ...
    this.pageInfo$.subscribe(data => {
      ...
      if (this.clientData !== data.contacts) {
        this.clientData = data.contacts;
      ...
      if (!this.initialFilterSortSet) {
        ...
        setTimeout(() => {
          ...
          if (data.caregiver) {
            this.SocialSecurityNum = data.caregiver;
          }
        }, 200);
      }
    });
  ...
  generateClientsCaregivers() {
    let caregivers = this.commonCaregiveres.map(c => ({
  ...
  createDynamicColumn() {
    let tempColumnDefs = [];
    if (localStorage.getItem('caregiver_contacts_columns')) {
      tempColumnDefs = JSON.parse(localStorage.getItem('caregiver_contacts_columns'));
    }
  ...
  getAll() {
    ...
    this.store.dispatch(new LoadCaregiverContacts({SocialSecurityNum: this.SocialSecurityNum}));
  }
  ...
  updateTableColShowHide(content) {
    // localStorage.setItem('caregiver_contacts_columns', JSON.stringify(this.temp_show_hide_array));
    ...
    localStorage.setItem('caregiver_contacts_columns', JSON.stringify(this.columnDefs));
  ...
  addNewClient() {
    this.router.navigate(['/client/details/0']);
  }
  ...
  downloadCsvFile() {
    ...
    if (window.navigator.msSaveOrOpenBlob) window.navigator.msSaveBlob(blob, 'ClientCSVReport.csv');
    else {
      var a = window.document.createElement('a');
      a.href = window.URL.createObjectURL(blob);
      a.download = 'ClientCSVReport.csv';
  ...
  setUserStatusType(statusFilter) {
    this.store.dispatch(new SetCaregiverContactsPageInfo({statusFilter}));
  }
  ...
  updateColumnDefsFromTable() {
    ...
    localStorage.setItem('caregiver_contacts_columns', JSON.stringify(tempColumnDefs));
  ...
  paginationChanged() {
    ...
    this.store.dispatch(new SetCaregiverContactsPageInfo({pageSize, currentPage}));
  }

  filterChanged() {
    let filter: any = this.agGrid.api.getFilterModel();
    this.store.dispatch(new SetCaregiverContactsPageInfo({filter}));
    this.agGrid.api.refreshCells();
  }

  sortChanged() {
    let sort: any = this.agGrid.api.getSortModel();
    this.store.dispatch(new SetCaregiverContactsPageInfo({sort}));
    this.agGrid.api.refreshCells();
  }
