...
import {LoadClients, SetClientListPageInfo} from '../../states/actions/client.actions';
...
export class ListComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    // localStorage.removeItem('client_columns');
  ...
  ngOnInit() {
    ...
    if (!localStorage.getItem('client_columns')) {
    ...
    this.pageInfo$ = this.store.pipe(
      takeUntil(this._unsubscribeAll),
      select(fromRoot.getClientListPageInfo)
    );
  ...
  createDynamicColumn() {
    let tempColumnDefs = [];
    if (localStorage.getItem('client_columns')) {
      tempColumnDefs = JSON.parse(localStorage.getItem('client_columns'));
    }
  ...
  updateTableColShowHide(content) {
    // localStorage.setItem('client_columns', JSON.stringify(this.temp_show_hide_array));
    ...
    localStorage.setItem('client_columns', JSON.stringify(this.columnDefs));
  ...
  setUserStatusType(statusFilter) {
    this.store.dispatch(new SetClientListPageInfo({statusFilter}));
  }
  ...
  updateColumnDefsFromTable() {
    ...
    localStorage.setItem('client_columns', JSON.stringify(tempColumnDefs));
  ...
  paginationChanged() {
    ...
    this.store.dispatch(new SetClientListPageInfo({pageSize, currentPage}));
  }

  filterChanged() {
    let filter: any = this.agGrid.api.getFilterModel();
    this.store.dispatch(new SetClientListPageInfo({filter}));
    this.agGrid.api.refreshCells();
  }

  sortChanged() {
    let sort: any = this.agGrid.api.getSortModel();
    this.store.dispatch(new SetClientListPageInfo({sort}));
    this.agGrid.api.refreshCells();
  }
