...
@Component({
  selector: 'app-master-list-physician',
  templateUrl: './physician.component.html',
  styleUrls: ['./physician.component.scss']
})
export class PhysicianMasterListComponent implements OnInit {
  ...
  addNewData() {
    this.store.dispatch(new CreateCommonData({target: 'physicians', data: this.editData}));
  }
  ...
  updateData() {
    this.store.dispatch(
      new UpdateCommonData({
        target: 'physicians',
        idInfo: [this.editRow.PhysicianName],
        idFields: ['PhysicianName'],
        data: this.editData
      })
    );
  }
