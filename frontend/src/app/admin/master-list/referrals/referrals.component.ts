...
const OMIT_FIELDS = [...COMMON_OMIT_FIELD, 'ReferredById'];

@Component({
  selector: 'app-master-list-referrals',
  templateUrl: './referrals.component.html',
  styleUrls: ['./referrals.component.scss']
})
export class ReferralMasterListComponent implements OnInit {
  ...
  addNewData() {
    this.store.dispatch(new CreateCommonData({target: 'referredBy', data: this.editData}));
  }
  ...
  updateData() {
    this.store.dispatch(
      new UpdateCommonData({
        target: 'referredBy',
        idInfo: [this.editRow.ReferredById],
        idFields: ['ReferredById'],
        data: this.editData
      })
    );
  }
