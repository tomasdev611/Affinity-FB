...
const OMIT_FIELDS = [...COMMON_OMIT_FIELD, 'ClassificationID'];

@Component({
  selector: 'app-master-list-classification',
  templateUrl: './classification.component.html',
  styleUrls: ['./classification.component.scss']
})
export class ClassificationMasterListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.store
      ...
      .subscribe(data => {
        if (data.loaded) {
          this.listData = data.classifications;
        }
      });
  }
  ...
  addNewData() {
    this.store.dispatch(new CreateCommonData({target: 'classifications', data: this.editData}));
  }
  ...
  updateData() {
    this.store.dispatch(
      new UpdateCommonData({
        ...
        idInfo: [this.editRow.ClassificationID],
        idFields: ['ClassificationID'],
        data: this.editData
      })
    );
  }
