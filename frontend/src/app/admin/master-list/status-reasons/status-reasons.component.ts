...
const OMIT_FIELDS = ['dtm_created', 'dtm_lastUpdated', 'str_createdBy', 'str_updatedBy'];

@Component({
  selector: 'app-master-list-status-reasons',
  templateUrl: './status-reasons.component.html',
  styleUrls: ['./status-reasons.component.scss']
})
export class ReasonsMasterListComponent implements OnInit {
  ...
  addNewData() {
    this.store.dispatch(new CreateCommonData({target: 'reasons', data: this.editData}));
  }
  ...
  updateData() {
    this.store.dispatch(
      new UpdateCommonData({
        target: 'reasons',
        idInfo: [this.editRow.str_reason],
        idFields: ['str_reason'],
        data: this.editData
      })
    );
