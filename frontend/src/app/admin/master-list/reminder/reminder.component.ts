...
@Component({
  selector: 'app-master-list-reminders',
  templateUrl: './reminder.component.html',
  styleUrls: ['./reminder.component.scss']
})
export class ReminderMasterListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.store
      ...
      .subscribe(data => {
        if (data.loaded) {
          this.listData = data.reminderDescriptions;
        }
      });
  }
  ...
  addNewData() {
    this.store.dispatch(
      new CreateCommonData({target: 'reminderDescriptions', data: this.editData})
    );
  }
  ...
  updateData() {
    this.store.dispatch(
      new UpdateCommonData({
        target: 'reminderDescriptions',
        idInfo: [this.editRow.description],
