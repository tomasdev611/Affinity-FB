...
@Component({
  selector: 'app-master-list-note-type-caregiver',
  templateUrl: './note-type-caregiver.component.html',
  styleUrls: ['./note-type-caregiver.component.scss']
})
export class NoteTypeCaregiverMasterListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.store
      ...
      .subscribe(data => {
        if (data.loaded) {
          this.listData = data.caregiverNoteTypes;
        }
      });
  }
  ...
  addNewData() {
    this.store.dispatch(new CreateCommonData({target: 'caregiverNoteTypes', data: this.editData}));
  }

  updateData() {
    this.store.dispatch(
      new UpdateCommonData({
        target: 'caregiverNoteTypes',
