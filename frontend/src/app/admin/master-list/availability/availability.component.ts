...
@Component({
  selector: 'app-master-list-availability',
  templateUrl: './availability.component.html',
  styleUrls: ['./availability.component.scss']
})
export class AvailabilityMasterListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.store
      ...
      .subscribe(data => {
        if (data.loaded) {
          this.listData = data.availabilities;
        }
      });
  }
  ...
  addNewData() {
    this.store.dispatch(new CreateCommonData({target: 'availabilities', data: this.editData}));
  }
