...
const DEFAULT_DATA = {
  Description: '',
  ItemTypeId: 1 // TODO: This seems temporary solution. Should either remove foreign key constraint or should get item type
};

const OMIT_FIELDS = [...COMMON_OMIT_FIELD, 'ServiceCode', 'itemDescription'];

@Component({
  selector: 'app-master-list-services',
...
export class ServiceMasterListComponent implements OnInit {
  ...
  addNewData() {
    this.store.dispatch(new CreateCommonData({target: 'services', data: this.editData}));
  }
  ...
  updateData() {
    this.store.dispatch(
      new UpdateCommonData({
        target: 'services',
        idInfo: [this.editRow.ServiceCode],
        idFields: ['ServiceCode'],
        data: this.editData
      })
    );
  }
