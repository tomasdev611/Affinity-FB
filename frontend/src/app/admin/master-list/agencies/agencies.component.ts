...
@Component({
  selector: 'app-master-list-agencies',
  templateUrl: './agencies.component.html',
  styleUrls: ['./agencies.component.scss']
})
export class AgenciesMasterListComponent implements OnInit {
  ...
  addNewData() {
    this.store.dispatch(new CreateCommonData({target: 'agencies', data: this.editData}));
  }
  ...
  updateData() {
    this.store.dispatch(
      new UpdateCommonData({
        target: 'agencies',
        idInfo: [this.editRow.AgencyName],
        idFields: ['AgencyName'],
        data: this.editData
      })
    );
