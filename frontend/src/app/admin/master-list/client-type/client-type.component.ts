...
const OMIT_FIELDS = [...COMMON_OMIT_FIELD, 'clientTypeID'];

@Component({
  selector: 'app-master-list-clienttype',
  templateUrl: './client-type.component.html',
  styleUrls: ['./client-type.component.scss']
})
export class ClientTypeMasterListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.store
      ...
      .subscribe(data => {
        if (data.loaded) {
          this.listData = data.clientTypes;
        }
      });
  }
  ...
  addNewData() {
    this.store.dispatch(new CreateCommonData({target: 'clientTypes', data: this.editData}));
  }
  ...
  updateData() {
    this.store.dispatch(
      new UpdateCommonData({
        target: 'clientTypes',
        idInfo: [this.editRow.clientTypeID],
        idFields: ['clientTypeID'],
        data: this.editData
      })
    );
  }
