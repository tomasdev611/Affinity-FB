...
import ApplicantImage from './ApplicantImageModel';
...
export class ApplicantImageService {
  ...
  async syncAttachmentsToS3() {
    try {
      let securityNums = await ApplicantImage.query(this.knex).select('SocialSecurityNum');
      let tryCount = securityNums.length / 100;
      console.log('ApplicantImageService syncAttachmentsToS3 Start', tryCount, securityNums.length);
      for (let i = 0; i < tryCount; i++) {
        const socialSecurityNums = securityNums
          .slice(i * 100, (i + 1) * 100)
          .map(c => c.SocialSecurityNum);

        let attachments = await ApplicantImage.query(this.knex).whereIn(
          'SocialSecurityNum',
          socialSecurityNums
        );
        for (let attachment of attachments) {
          // fs.writeFileSync(
          //   `./tmp/applicant_photos/${attachment.SocialSecurityNum}.jpg`,
      ...
      console.log('ApplicantImageService syncAttachmentsToS3 complete');
    } catch (error) {
  ...
  async removeFilesForAllCaregivers() {
    await ApplicantImage.query(this.knex).delete();
  }
  ...
  //   let attachments = await ApplicantImage.query(this.knex)
  ...
  //     await ApplicantImage.query(this.knex)
