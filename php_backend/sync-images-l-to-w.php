...
$sql = "select * from dbo.employee where lastUpdated > dateadd(day,-30, getdate())";
...
foreach( $rows as $key=>$row) {
	...
	if ($ssn && $image_row ['SocialSecurityNum'] && $image_row ['bin_image']) {
		$file = file_get_contents ( "http://54.166.218.128/scripts/sync-images-l-to-w-2.php?id=" . $ssn );
		file_put_contents ( 'employees/images/img' . $ssn . '.jpeg', $file );
		saveToS3 ( $s3, "img" . $ssn.'.jpeg', 'employees/images/img' . $ssn . '.jpeg' );
		$temp [] = $ssn;
